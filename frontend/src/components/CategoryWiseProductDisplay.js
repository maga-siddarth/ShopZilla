...
//Inside the Product as Recommeded Product
  ...
  const[scroll,setScroll] = useState(0)
    ...
    const fetchdata = async()=>{
        setLoading(true)
            ...
            setLoading(false)
            console.log("horrixontaldata",categoryProduct.data);
    ...
    loadingList.map((product,index)=>{
        return(
            ...
            <div className="p-4 grid gap-3">
             ...
             <button className=" text-sm text-white rounded-full px-3   animate-pulse  bg-slate-200 w-full py-2"></button>
            </div>
          </div>
        )
    })
